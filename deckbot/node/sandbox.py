...
def build_command(
  ...
  """Return the argv list for a MYSTRAN run under valgrind.

  Runs MYSTRAN directly under valgrind memcheck (no firejail wrapper —
  valgrind is incompatible with firejail).  The process should be
  launched with ``cwd`` set to the run's work directory so that MYSTRAN
  writes all output files there.
